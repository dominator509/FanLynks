...
('usr_dev_admin', 'admin@example.com', 'REPLACE_WITH_ADMIN_PASSWORD_HASH', '${now}', '${now}', NULL, 1);

INSERT INTO tenants (id, owner_user_id, name, slug, created_at, updated_at) VALUES
('tenant_demo', 'usr_dev_admin', 'Demo Tenant', 'demo-tenant', '${now}', '${now}');
...
('page_demo', 'tenant_demo', 'demo', 'Demo Link Hub', 'A fast, high-conversion one-page link hub.', NULL, 1, 'New offer is live', 'https://example.com/launch', 'Top Offer', 'https://example.com/offer', 'ga4_meta', 'default_standard', 1, 1, '${now}', '${now}',
...
('link_offer', 'page_demo', NULL, 'VIP Offer', 'Highest-value CTA first', 'https://example.com/vip', 'emoji', '🔥', 'Top pick', 'primary', 1, 1, NULL, NULL, '${now}', '${now}'),
('link_join', 'page_demo', NULL, 'Join Free List', 'Capture warm traffic before the sale', 'https://example.com/list', 'emoji', '✉️', NULL, 'secondary', 2, 1, NULL, NULL, '${now}', '${now}'),
('link_book', 'page_demo', NULL, 'Book a Call', 'High-intent visitors only', 'https://example.com/book', 'emoji', '📅', NULL, 'neutral', 3, 1, NULL, NULL, '${now}', '${now}');

INSERT INTO integrations (id, page_id, provider, config_json, is_enabled, created_at, updated_at) VALUES
('intg_ga4', 'page_demo', 'ga4', json_object('measurementId','G-REPLACE_ME','enablePageViews',1,'enableClickEvents',1,'enableExperimentParameters',1), 1, '${now}', '${now}'),
('intg_meta', 'page_demo', 'meta', json_object('pixelId','REPLACE_ME','eventMappings', json_object('page_view','PageView','hero_cta_click','ViewContent','link_click','ViewContent')), 0, '${now}', '${now}'),
('intg_gtm', 'page_demo', 'gtm', json_object('containerId','GTM-REPLACE','advertisingEnabled',0), 0, '${now}', '${now}'),
...
('exp_demo', 'page_demo', 'Demo palette test', 'paused', 30, NULL, NULL, NULL, '${now}', '${now}');
...
('var_black', 'exp_demo', 'Black Luxe', 50, json_object('bg','#09090b','surface','#18181b','text','#fafafa','accent','#d4af37'), json_object('page', json_object('title','Demo Link Hub')), 1, '${now}', '${now}'),
('var_pink', 'exp_demo', 'Pink Neon', 50, json_object('bg','#140917','surface','#2c1233','text','#fff7fb','accent','#ff4fd8'), json_object('page', json_object('title','Demo Link Hub')), 1, '${now}', '${now}');
`;
